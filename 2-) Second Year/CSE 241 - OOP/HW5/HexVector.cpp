...
{
    HexVector::HexVector()
        :HexVector(6, 6, "user vs computer")
    {/*Intentionally left empty*/}

    HexVector::HexVector(int heightValue, int widthValue)
        :HexVector(heightValue, widthValue, "user vs computer")
    {/*Intentionally left empty*/}

    HexVector::HexVector(int heightValue, int widthValue, string gameTypeInfo)
        :AbstractHex(heightValue, widthValue, gameTypeInfo)
    {
        setBoard();
    }

    ostream& operator<<(ostream& outputStream, const HexVector& current)
    {
        int l;
    ...
    bool HexVector::operator==(const AbstractHex& other)const
    {
        try
        {
            if(this==&other)
    ...
    char HexVector::operator()(int indexi, int indexj)const
    {
    ...
    void HexVector::readFromFile(string file)
    {
    ...
    //Throws CannotOpenFileException if cannot open the given file
    void HexVector::writeToFile(string file)const
    {
    ...
    void HexVector::controlCell(int chosenRow, char chosenColumn)
    {
    ...
    const Cell& HexVector::play(Cell cMove)
    {
        try{
            if(whoseTurn=="User1" || whoseTurn=="User")
            {
    ...
    bool HexVector::neighbourCheck(int startRow, char startColumn, CellState stateInfo, int& pastCount, vector<Cell> &pastCells)const
    {
        Cell startCell;
