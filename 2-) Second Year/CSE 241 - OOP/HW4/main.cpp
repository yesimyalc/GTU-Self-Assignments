...
int main()
{
    ...
    "-To reset a move enter --UNDO or UNDO-- commands."<<endl<<
    "-To see your score, use SCORE command."<<endl<<endl;
    cout<<"**************************************************"<<endl<<endl;
    ...
    //This loop will keep the games being played until the player uses -1 command to exit the games or till all the games finish
    while(Hex::currentGameNo!=-1)
    {
        ...
        if(hexGames[Hex::currentGameNo-1].getCommand()=="COMPARE")
        {
            ...
            if(hexGames[Hex::currentGameNo-1]==hexGames[compare-1])
