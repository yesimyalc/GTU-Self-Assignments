#ifndef HEXADAPTER_H_INCLUDED
#define HEXADAPTER_H_INCLUDED
...
{
    template< template<class, class> class T> class HexAdapter;
    template< template<class, class> class T> ostream& operator<<(ostream& outputStream, const HexAdapter<T>& current);
    ...
    public:
        //Constructors
        HexAdapter();
        HexAdapter(int heightValue, int widthValue);
        HexAdapter(int heightValue, int widthValue, string gameTypeInfo);

        //Only the destructor is needed
        virtual ~HexAdapter(){}

        //Overloaded Operators
        friend ostream& operator<< <>(ostream& outputStream, const HexAdapter<T>& current);
    ...
    private:
        T< T<Cell, allocator<Cell> >, allocator<T<Cell, allocator<Cell> > > > hexCells;
    ...
    HexAdapter<T>::HexAdapter()
        :HexAdapter(6, 6, "user vs computer")
    {/*Intentionally left empty*/}

    template< template<class, class> class T>
    HexAdapter<T>::HexAdapter(int heightValue, int widthValue)
        :HexAdapter(heightValue, widthValue, "user vs computer")
    {/*Intentionally left empty*/}

    template< template<class, class> class T>
    HexAdapter<T>::HexAdapter(int heightValue, int widthValue, string gameTypeInfo)
    ...
    template< template<class, class> class T>
    ostream& operator<<(ostream& outputStream, const HexAdapter<T>& current)
    {
        int l;
    ...
    template< template<class, class> class T>
    char HexAdapter<T>::operator()(int indexi, int indexj)const
    {
        if(indexi<0 || indexi>getBoardHeight() || indexj<0 || indexj>getBoardWidth())
            throw InvalidBoardCellException();

        return static_cast<char>(hexCells[indexi][indexj].getCellState());
    }

    //Initializes the board
    template< template<class, class> class T>
    void HexAdapter<T>::setBoard()
    {
        hexCells.resize(getBoardHeight());//Resizes the board according to the height and width values
        for(int i=0; i<getBoardHeight(); ++i)
    ...
    template< template<class, class> class T>
    void HexAdapter<T>::readFromFile(string file)
    {
    ...
    //Throws CannotOpenFileException if cannot open the given file
    template< template<class, class> class T>
    void HexAdapter<T>::writeToFile(string file)const
    {
    ...
    template< template<class, class> class T>
    void HexAdapter<T>::controlCell(int chosenRow, char chosenColumn)
    {
        ...
        for(int i=1; i<7; ++i)
        {
            ...
            try
            {
                ...
                if(doneMoves==0 && (lastMove().getNeighbourCell(j)).getNeighbourCellRow()>0 && (lastMove().getNeighbourCell(j)).getNeighbourCellRow()<=height &&
                  ...
                  hexCells[(lastMove().getNeighbourCell(j)).getNeighbourCellRow()-1][static_cast<char>((lastMove().getNeighbourCell(j)).getNeighbourCellColumn())-65].getCellState()==CellState::Empty)
                {
                    moveCount++;
                    ...
                    print();
                    hexCells[(lastMove().getCellRow())-1][static_cast<char>(lastMove().getCellColumn())-65].updateCellState('O');//The board is being updated
    ...
    template< template<class, class> class T>
    const Cell& HexAdapter<T>::play(Cell cMove)
    {
        try{
            if(whoseTurn=="User1" || whoseTurn=="User")
            {
    ...
    //Used in isEnd. Checks if there is a path that leads to a win condition. Returns true if there is
    template< template<class, class> class T>
    bool HexAdapter<T>::neighbourCheck(int startRow, char startColumn, CellState stateInfo, int& pastCount, vector<Cell> &pastCells)const
    {
        Cell startCell;
